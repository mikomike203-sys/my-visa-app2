...
import { Eye, EyeOff, Landmark, WalletCards } from "lucide-react";
import type { CardColor } from "./Card3D";
...
export function RegisterPage({ onSwitchToLogin }: Props) {
  ...
  const [cardLook, setCardLook] = useState<CardColor>("graphite");
      ...
      <div className="mx-auto grid min-h-[calc(100dvh-4rem)] w-full max-w-5xl items-center gap-8 lg:grid-cols-[1fr_420px]">
        <div className="hidden lg:block">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-black bg-[#d7ff5f] px-4 py-2 text-xs font-black">
            <WalletCards className="h-4 w-4" /> Wallet ready in seconds
          </div>
          <h1 className="text-6xl font-black leading-[0.95] tracking-tight">Start free with Visa Kenya.</h1>
          <p className="mt-5 max-w-lg text-base font-medium leading-7 text-zinc-600">
            Create your wallet, get a starter card, then send and receive with QR-ready flows.
          </p>
        </div>
        ...
        <div className="rounded-[28px] border border-black bg-white p-6 shadow-[8px_8px_0_#000]">
          <div className="mb-8 text-center">
            ...
            <h2 className="text-3xl font-black tracking-tight">Create wallet</h2>
            <p className="mt-2 text-sm font-medium text-zinc-500">Your first card is created automatically</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <label className="block">
              <span className="mb-1.5 block text-xs font-black uppercase text-zinc-600">Full name</span>
              <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="John Doe" autoComplete="name" required className="w-full rounded-2xl border border-black bg-white px-4 py-3 font-bold text-black outline-none focus:shadow-[3px_3px_0_#000]" />
            </label>
            <label className="block">
            ...
            <label className="block">
              ...
              <div className="relative">
                <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 6 characters" autoComplete="new-password" required minLength={6} className="w-full rounded-2xl border border-black bg-white px-4 py-3 pr-11 font-bold text-black outline-none focus:shadow-[3px_3px_0_#000]" />
            ...
            <div>
              <span className="mb-2 block text-xs font-black uppercase text-zinc-600">Choose first card</span>
              <div className="grid grid-cols-5 gap-2">
                {[
                  ["graphite", "bg-zinc-900"],
                  ["sky", "bg-blue-600"],
                  ...
                  <button
                    type="button"
                    key={value}
                    onClick={() => setCardLook(value as CardColor)}
                    className={`h-11 rounded-2xl border border-black ${swatch} ${cardLook === value ? "ring-2 ring-[#d7ff5f] ring-offset-2" : ""}`}
          ...
          <p className="mt-6 text-center text-sm font-bold text-zinc-600">
            Already have a wallet? <button onClick={onSwitchToLogin} className="text-black underline">Sign in</button>
          </p>
        </div>
      </div>
    </div>
